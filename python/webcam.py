...
class FireSmokeDetection(object):
    ...
    def draw(self, img, boxinfo):
        for xyxy, conf, cls in boxinfo:
            print('conf: ', conf)
            self.plot_one_box(xyxy, img, label=f"{self.classes[int(cls)]} {conf:.2f}", color=self.colors[int(cls)], line_thickness=2)
        ...
        for batch_id in range(self.batchsize):
            if self.grid:
                ...
                output.append(self.sigmoid(
                    infer_request.get_output_tensor(0).data[batch_id].reshape(-1, self.size[0] * 3,
                                                                              5 + self.class_num)))
                output.append(self.sigmoid(
                    infer_request.get_output_tensor(1).data[batch_id].reshape(-1, self.size[1] * 3,
                                                                              5 + self.class_num)))
                output.append(self.sigmoid(
                    infer_request.get_output_tensor(2).data[batch_id].reshape(-1, self.size[2] * 3,
                                                                              5 + self.class_num)))
...
# Create object for the class FireSmokeDetection
detector = FireSmokeDetection(model, device, pre_api, batchsize, nireq, grid)
detector.infer_cam(input)
